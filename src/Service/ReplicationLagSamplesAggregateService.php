...
namespace BlackCat\Database\Packages\ReplicationLagSamples\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\ReplicationLagSamples\Dto\ReplicationLagSampleDto;
use BlackCat\Database\Packages\ReplicationLagSamples\Mapper\ReplicationLagSampleDtoMapper;
 ...
 */
final class ReplicationLagSamplesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private ReplicationLagSampleRepository $replicationLagSampleRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'replication_lag_samples' => $this->replicationLagSampleRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\ReplicationLagSamples\ModuleException("Record {$idStr} not found for locking.");
            }
