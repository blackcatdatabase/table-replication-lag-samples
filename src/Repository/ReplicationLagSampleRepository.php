...
namespace BlackCat\Database\Packages\ReplicationLagSamples\Repository;
...
use BlackCat\Database\Packages\ReplicationLagSamples\Criteria;
use BlackCat\Database\Packages\ReplicationLagSamples\Dto\ReplicationLagSampleDto as Dto;
use BlackCat\Database\Packages\ReplicationLagSamples\Mapper\ReplicationLagSampleDtoMapper as RowMapper;
...
final class ReplicationLagSampleRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\ReplicationLagSamples\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\ReplicationLagSamples\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\ReplicationLagSamples\Dto\ReplicationLagSampleDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\ReplicationLagSamples\Dto\ReplicationLagSampleDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\ReplicationLagSamples\Mapper\ReplicationLagSampleDtoMapper::fromRow($row);
    }
